...
- ./data/(과목명)/*.pdf  ← 과목별 자료
- ./data/(과목명)/*.md
- ./data/*.pdf            ← 과목 횡단 자료 (여러 과목에 걸친 통합 파일이 존재할 수 있음)
...
- **시나리오 데이터**: `data/시나리오.pdf`를 시나리오 데이터로 취급한다. 실제 시험에 출제될 시나리오를 복원한 것으로, 시험 직전 모드에서는 최우선 데이터이다.

## 시험 직전 모드 (Exam-Specific Override)
> **유효 기간**: 2026년 2월 10일 시험 종료 시까지
> **이후**: 이 섹션을 삭제하고 기존 데이터 처리 우선순위로 복원한다.

### 시나리오 데이터
- `data/시나리오.pdf` — 복원된 실제 시험 시나리오 (문제은행 기반)
- 이 파일이 존재하는 동안, 출력물은 시나리오의 구체적 내용(환자 정보, 모의대화, 자가채점표)을 **최우선으로 반영**한다.
- 기존 자가채점표, 강의자료, 영상 요약본은 시나리오를 보충하는 역할로 활용한다.

### 데이터 처리 우선순위 (시험 직전 모드)
1. **시나리오.pdf** — 실제 시험에 출제될 시나리오이므로 최우선 반영
2. **자가채점표** — 시나리오의 자가채점표와 기존 자가채점표를 통합하되, 충돌 시 시나리오.pdf 우선
3. **강의자료 및 영상 요약본** — 보충 배경지식
4. **Claude 의학 지식** — 위 자료에 없는 부분 보충

### 출력 목표 변경
- 기존: 어떤 CASE가 나올지 모르므로 범용 대비
- 시험 직전 모드: 특정 시나리오에 맞춘 **실전 수행 최적화**
  - 환자 인적사항, Vital Sign 등 시나리오 구체 정보 포함
  - 모의 대화 (질문-답변) 형태로 문진 내용 구성
  - 감별진단보다 **원활한 진료 수행 흐름**에 초점
...
- **체크리스트** 기능: 채점 항목 등을 클릭하여 체크 가능하도록
  - **data-id 규칙**: `{version}-{section}-{number}` 형식 (예: `v1-musculo-1`)
  - 콘텐츠가 크게 변경되면 version 프리픽스를 변경하여 기존 localStorage 데이터와 분리
- 인쇄 지원은 불필요
...
- CASE명이 변경되면 다음을 모두 갱신할 것:
  1. CLAUDE.md 과목별 CASE 목록 테이블
  2. `outputs/CPX_YYYYMMDD.html` (탭 라벨, 본문, DDx 참조 등)
  3. `YYYY-MM-DD/index.html` (outputs 복사본)
  4. 루트 `index.html`의 desc 텍스트
...
## 환경 참고사항
- **OS**: Windows (기본 인코딩 cp949)
- **PDF 읽기**: Read 도구의 내장 PDF 리더를 우선 사용 (`pages` 파라미터로 페이지 범위 지정). 내장 리더 실패 시에만 PyMuPDF(fitz) 등 외부 도구를 사용하되, sys.stdout을 UTF-8로 래핑할 것
- **Node.js**: 설치됨 (npm 패키지 활용 가능)
